...
// tidak pernah dikirim ke browser -- browser mengirim RINGKASAN keuangan + pertanyaan
// bebas kalau ada, dan menerima balasan JSON dari Gemini.
//
// PENTING (v134, temuan T6): "ringkasan" itu SEBAGIAN BESAR angka agregat, tapi TIDAK
// semuanya. src/domain/ai-summary.js juga menyertakan, untuk 3 transaksi pengeluaran
// terbesar bulan ini (field `transaksi_terbesar_bulan_ini`): NAMA AKUN dan `keterangan`
// (catatan bebas milik pengguna, dipotong 80 karakter). Keduanya ikut terkirim ke
// penyedia AI pihak ketiga (Google Gemini). Tidak ada API key, email, password, atau ID
// user yang dikirim. Kalau catatan transaksi dianggap terlalu pribadi, field itu harus
// dihapus/dimatikan di ai-summary.js -- jangan diasumsikan tidak terkirim.
//
...
// v134 (audit AI T1/T2/T3/T7): logika murni -- batas input, generationConfig, dan
// sanitasi balasan -- pindah ke modul .js bersama supaya bisa diuji di Node
// (tests/unit/ai-edge-output.test.js), bukan baru ketahuan setelah deploy.
import {
  MAX_QUESTION_LENGTH, GEMINI_TIMEOUT_MS, clampText, generationConfigFor,
  sanitizeInsights, sanitizeCategory,
} from "../_shared/ai-output.js";
...
// KOREKSI v134 (audit AI, temuan T6): komentar lama di sini menyebut fitur "terpanggil
// cukup sering (tiap dashboard dibuka / tiap ada transaksi baru, dengan jeda minimal
// 3 menit)". Itu TIDAK sesuai kode klien: Gemini hanya dipanggil saat pengguna menekan
// tombol refresh (ikon ⟳) di kartu Rekomendasi AI -- tidak pernah otomatis saat dashboard
// dibuka, dan tidak ada jeda 3 menit di klien (yang ada: rate limit server-side per jam
// di bawah + cache hasil di appSettings.ai_insight_cache). Model tier murah/cepat tetap
// pilihan yang masuk akal, tapi alasannya bukan frekuensi otomatis.
...
async function callGemini(promptText: string, mode: string) {
  let resp: Response;
  try {
    resp = await fetch(GEMINI_ENDPOINT, {
      ...
      body: JSON.stringify({
        ...
        // v134 (T2): pagu token keluaran + temperature rendah + responseMimeType
        // JSON utk mode yang balasannya wajib JSON. Sebelumnya tidak ada
        // generationConfig sama sekali, jadi biaya keluaran tidak terpagar dan
        // jawaban utk data yang sama tidak stabil.
        generationConfig: generationConfigFor(mode),
      }),
      // v134 (T3): tanpa batas waktu, panggilan Gemini yang menggantung menahan
      // function sampai batas platform dan UI menunggu tanpa kepastian.
    ...
    throw new Response(
      JSON.stringify({
        error: timeout
          ? "Gemini API tidak menjawab dalam batas waktu. Coba lagi beberapa saat."
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (mode === "suggest_category") {
      ...
      let rawText: string;
      try {
        rawText = await callGemini(catPrompt, "suggest_category");
      } catch (errResp) {
        if (errResp instanceof Response) return errResp;
        throw errResp;
      }
      // Jaga2: cuma terima kalau AI benar2 balas salah satu dari daftar yg dikirim,
      // JANGAN percaya buta -- kalau AI "mengarang" nama kategori baru, anggap
      // tidak ada saran. (v134: pindah ke _shared/ai-output.js, perilaku identik.)
      return jsonResponse({ kategori: sanitizeCategory(rawText, categories) });
    }

    // Ringkasan (dari buildFinanceSummaryForAI() di src/domain/ai-summary.js) sebagian besar
    // angka agregat per bulan/kategori -- bukan daftar transaksi mentah -- supaya payload kecil.
    // KOREKSI v134 (temuan T6): komentar lama menyebut ringkasan ini "tidak membocorkan detail
    // transaksi individual (keterangan, dll)". Itu tidak tepat: field `transaksi_terbesar_bulan_ini`
    // MEMBAWA nama akun + keterangan (80 karakter) untuk 3 transaksi terbesar. Sisanya agregat.
    const commonContext =
    ...
    if (mode === "monthly_summary") {
      ...
      try {
        summaryText = await callGemini(summaryPrompt, "monthly_summary");
      } catch (errResp) {
        if (errResp instanceof Response) return errResp;
        throw errResp;
      }
    ...
    if (question && String(question).trim()) {
      ...
      // v134 (T1): pertanyaan bebas DIPOTONG sebelum masuk prompt. Sebelumnya
      // String(question).trim() disisipkan apa adanya tanpa batas panjang --
      // satu-satunya pagar hanyalah rate limit 40/jam.
      const questionText = clampText(question, MAX_QUESTION_LENGTH);
      const qaPrompt = commonContext +
        `User bertanya: "${questionText}"\n\n` +
      ...
      try {
        answerText = await callGemini(qaPrompt, "question");
      } catch (errResp) {
        if (errResp instanceof Response) return errResp;
        throw errResp;
      }
    ...
    let rawText: string;
    try {
      rawText = await callGemini(prompt, "insights");
    } catch (errResp) {
      if (errResp instanceof Response) return errResp;
      throw errResp;
    }

    // Sanitasi output (v65; v94 menambah field `detail`; v134: logika pindah ke
    // _shared/ai-output.js supaya bisa diuji di Node -- perilaku identik: buang
    // item tanpa title/message, potong 80/500/4000 karakter, severity whitelist,
    // maksimal 5 kartu, dan fallback satu kartu bila balasan bukan JSON).
    const insights = sanitizeInsights(rawText);
