...
-- ============================================================================
-- CATATAN AUDIT DRIFT LIVE-vs-REPO 2026-09-13 (v124) - BACA SEBELUM
-- MEMBANDINGKAN BENTUK TABEL. File ini ARSIP RIWAYAT dan sengaja TIDAK ditulis
-- ulang, jadi definisi tabel di bawah sudah TIDAK LAGI menggambarkan produksi:
--
--     file ini : id uuid primary key default gen_random_uuid()
--     PRODUKSI : id bigint generated by default as identity
--                (+ sequence platform_logos_id_seq)
--
-- Tabelnya sudah ada di database live SEBELUM file ini pernah dijalankan --
-- hampir pasti dibuat lewat Supabase Dashboard UI, yang otomatis memberi
-- `id bigint generated by default as identity`. Karena file ini memakai
-- `create table if not exists` dan `alter table add column if not exists`, ia
-- DIAM-DIAM JADI NO-OP: tidak pernah membentuk ulang tabelnya, dan tidak
-- pernah melaporkan error. Header asli di bawah sebenarnya sudah mengakui hal
-- ini ("tahan drift bentuk vs tabel live yang dibuat migrasi yang hilang
-- tadi"), tapi akibatnya terhadap schema.sql baru ketahuan saat audit 2026-09-13.
--
-- Acuan bentuk tabel sekarang: sql/schema.sql bagian 8, yang sejak v124
-- mengikuti produksi. Tiga objek lain yang dulu hanya ada di produksi ikut
-- diangkat ke sana: index platform_logos_active_idx, CHECK
-- platform_logos_key_format, dan trigger platform_logos_set_updated_at beserta
-- fungsinya.
--
-- Policy `platform_logos_read_authenticated` (qual: is_active = true) yang
-- disebut di butir (2) bawah adalah policy WARISAN dari migrasi yang hilang
-- itu. Ia redundan: policy permissive digabung dengan OR, dan "Platform logos
-- are publicly readable" sudah memakai `using (true)`, jadi policy warisan itu
-- tidak membatasi apa pun. DIHAPUS dari produksi pada 2026-09-13 tanpa
-- perubahan perilaku.
-- ============================================================================
