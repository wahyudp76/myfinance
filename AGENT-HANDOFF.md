...
## v114 — perbaikan bug menyeluruh + stability + performance (riwayat aset, akun bayangan, render dashboard 0-ms)
**KONTEKS:** permintaan lanjutan "perbaikan fix bug menyeluruh, cek stability dan peningkatan performance". Audit dilakukan dengan probe Playwright baru (tools/probe-bughunt.mjs, drive UI asli: form/modal/confirm + stub Supabase DINAMIS yang merefleksikan CRUD, bukan stub statis) + CPU profiler CDP (tools/probe-perf.mjs, tools/probe-profile.mjs; 2.500 transaksi, CPU 4x throttle). Dua bug nyata ditemukan DARI ALUR UI (bukan tebakan), keduanya diperbaiki + diverifikasi ulang E2E.

**BUG FIX #1 (dibuktikan probe): hapus aset yang masih punya transaksi setor -> "akun bayangan".** Setelah aset dihapus, transaksi setor yang tersisa (kategori = nama aset) tidak lagi dikenali resolveAssetDepositTx, lalu syncAccountsFromTransactions MENDAFTARKAN nama aset itu sebagai REKENING (probe: "Bibit" muncul di appSettings.accounts setelah deleteAssetData) -- persis penyakit "shopee merchant" v109, tapi dari pintu masuk berbeda (self-heal lama hanya memangkas nama yang cocok aset yang MASIH ada). Fix: deleteAssetData kini mendeteksi transaksi setor terkait (jumlah + total Rp di pesan konfirmasi) dan menghapus aset BERSAMA transaksi setornya (urutan: hapus transaksi dulu -> aset -> refreshTransactionsOnly(()=>refreshAssetsOnly()); tanpa patch penarikan karena asetnya memang dihapus; kegagalan hapus transaksi membatalkan penghapusan aset). Verifikasi E2E: konfirmasi menyebut "1 transaksi setor ke aset (total Rp 500.000)", "Bibit" TIDAK muncul di accounts, transaksi setor terhapus.

**BUG FIX #2 (dibuktikan probe): edit setor ke aset sambil mengganti TANGGAL meninggalkan titik value_history STALE.** Delta tunggal di-upsert di tanggal baru, titik di tanggal LAMA tetap memakai nilai lama -- grafik riwayat aset menampilkan lonjakan di tanggal yang sudah tidak punya transaksi (probe: setor 500rb @T1 lalu edit tanggal ke T2 -> T1 masih 1,5jt padahal deposit sudah pindah). Fix: fungsi murni baru applyAssetDepositEdit(asset, oldJumlah, newJumlah, oldTanggal, newTanggal) di src/domain/asset-flows.js -- bila tanggal berubah: "tarik" setoran lama di tanggal lama (titik dikoreksi turun) DULU, lalu terapkan setoran baru di tanggal baru; nilai/modal bersih tetap = newJumlah - oldJumlah (identik rumus lama bila tanggal tidak berubah). runDeposit di app.src.js memakainya untuk kasus aset tujuan sama. 4 unit test baru (termasuk repro persis bug + kasus pindah tanggal + ganti jumlah sekaligus + identitas dgn applyAssetDeposit utk setor baru).

**PERFORMANCE (profil CPU, bukan tebakan):** hotspot #1 pipeline = pembuatan ulang SELURUH render dashboard (angka animateRupiah + 4 chart Chart.js + leaderboard + kartu akun + kartu wawasan) pada SETIAP masuk tab dashboard / resize, padahal tidak ada yang berubah (~190-230ms per render pada 2.500 tx, CPU 4x; v112 sudah menghilangkan versi tersembunyinya, v114 menuntaskan sisa jalurnya):
- processDataForUI kini punya signature kesegaran (identitas REFERENSI globalData/globalAssets/appSettings.accounts/currentMonthBudgetsCache + themeAccentColor + nominalHidden + tanggal hari ini). Semua titik mutasi state di app ini berbentuk REASSIGN (diaudit) -> bandingkan referensi = aman. Bila dashboard terlihat & sig tidak berubah sejak render terakhir yang SUKSES (sig ditandai di AKHIR render, bukan awal -- gagal di tengah tidak pernah mentok di DOM setengah jadi): render di-skip total (chart & DOM masih hidup dan benar). Hasil terukur: re-entry dashboard tanpa perubahan 270ms -> 0.0ms, 0 instance Chart.js dibuat.
- Pemanggil yang mengubah state secara IN-PLACE (gaya kategori, ikon akun, hidden/custom categories, palet chart, refresh harga aset yang memutasi baris aset langsung) kini memaksa render lewat argumen kedua processDataForUI(globalData, true) -- 8 call-site ditandai eksplisit (audit lengkap semua 16 call-site).
- Jalur dashboard TERSEMBUNYI (echo simpan transaksi dari tab lain): kini SEMUA penulisan DOM dashboard dilewati (v112 baru chart-nya) -- cukup agregasi + insightsCtx; lastInsightsCtx tetap diperbarui (dipakai notifikasi ambang budget & rekomendasi AI), dan jalur ini sengaja TIDAK menandai sig sehingga switchView('dashboard') selalu render penuh tepat sebelum terlihat (pola guard v112 diperluas, bukan perilaku baru).
- Profil juga mengonfirmasi sisa biaya filterTransactions (~300-500ms pada 2.500 tx, 4x throttle) = biaya parse innerHTML ~6.000 node DOM + layout (150 baris) -- inherent, SENGAJA tidak diubah (kesimpulan sama dgn v112; virtualisasi di luar scope).

**HASIL TERUKUR (probe 2.500 tx, CPU 4x throttle):** re-entry dashboard tanpa perubahan 270ms -> 0.0ms (0 chart dibuat); sweep 270x switchView (tanpa throttle) 4.272ms -> 3.408ms; echo-save dari tab Transaksi 1.381ms (dari 1.499ms di run basis dgn dataset lebih besar); boot -> appShell 1.930ms. Stability: heap JS DATAR (9,5MB -> 9,5MB) setelah 270 switchView + 3 reload dgn SW aktif; 0 error halaman/console di seluruh alur CRUD (create/edit/delete semua jenis, transfer RPC, setor ke aset create/edit-tanggal/edit-jumlah/delete, hapus aset ber-transaksi, search kosong, empty state).

**VERIFIKASI (Node v22.23.2):** lint 0 masalah; unit 912/912 (4 test baru applyAssetDepositEdit); parity lokal 1/1 (5x beruntun, deterministik); sembilan harness browser 231/231; build app/boot idempoten; snapshot SW cocok. CACHE_VERSION `myfinance-v144` -> `myfinance-v145`.
...
## v117 — segmen donat pop-out tak lagi terpotong kotak kanvas + glow ter-clip lingkaran
**KONTEKS:** laporan pemilik atas v116: ketika satu segmen (parameter) chart pie diklik, segmen MEMBESAR tetapi bentuknya "seperti terpotong mengotak di ujung-ujung sisinya" — minta box outline pie diperbaiki.

**AKAR MASALAH (repro piksel + forensik vendor):** kanvas donat sangat kecil (w-24/w-20 CSS = 96/80 px mobile, 176 px catSubDonut) dan chartArea-nya = kanvas penuh (legend display:false, tanpa padding). Chart.js 4.5.1 TIDAK menganggarkan `hoverOffset` level-dataset dalam radius luar — `getMaxOffset()` resolve data-element options TANPA scope 'active' sehingga `hoverOffset: 8` terbaca 0 (terbukti empiris: radius = (min−spacing)/2 persis, 96px → 45). Akibatnya segmen aktif digeser +8px MELEWATI tepi kanvas (45+8=53 > 48) dan terpotong batas PERSEGI kanvas; ditambah `hudDonutGlowPlugin` shadowBlur 16 yang meluber → halo violet terpotong kotak (terlihat sebagai "box outline") bahkan saat idle. Probe repro (tools/probe-donut-clip-repro.mjs): SEMUA 6 donat × semua segmen menempel piksel solid di keempat tepi kanvas.

**PERUBAHAN (ukuran visual donat TIDAK berubah — dipilih agar desain radar tak bergeser):**
- `styles.src.css`: kelas baru `.donut-canvas-wrap { position: absolute; inset: -8px; }` (pola sama dgn .hud-radar yang memang sudah inset -7px; absolut → nol dampak layout tetangga).
- `index.html` (5 kanvas) + `src/ui/categories.js` (catSubDonut): kanvas donat dibungkus wrapper tsb → kanvas meluber 8px ke semua arah (96→112, 80→96, 176→192, dst).
- `layout: { padding: 8 }` di keenam config donat (`buildAssetDonutConfig` + `buildCategoryDonutConfig` di src/ui/charts.js; inline assetAlloc di src/ui/assets.js; accCat di src/ui/accounts.js; catSubDonut di src/ui/categories.js). Matematika: wrapper +8px per sisi − padding 8px per sisi → radius dasar = (kotakRadar−6)/2 = PERSIS ukuran lama (45/37/85/61/45/101 terverifikasi probe); ujung segmen aktif = base+8 = 53/45/93 ≤ setengah kanvas 56/48/96 → muat dgn slack 3px.
- `src/domain/chart-hud.js` `hudDonutGlowPlugin`: sebelum dataset digambar, ctx di-CLIP ke lingkaran konsentris beradius jarak pusat-donat ke tepi kanvas terdekat −0.5 → glow dipangkas mengikuti LINGKARAN konsentris (terbaca pudar alami), bukan garis kotak. Segmen donat selalu di dalam lingkaran clip. Guard chart tanpa meta/ukuran (tetap save+glow, tanpa crash).

**PERILAKU:** hover/ketuk segmen → tetap MEMBESAR (hoverOffset 8 dipertahankan), kini utuh tanpa potongan; kartu eksternal v115, suppress tooltip internal v116, klik-navigasi detail kategori — semua tak berubah.
...
## v122 — drift dokumen diperbaiki menyeluruh + guard `docs-consistency` diperluas 6 → 16 test
**KONTEKS:** audit baca-seluruh repo (2026-09-12) menemukan sederet klaim dokumen yang sudah lama tidak cocok dengan kode, dan — yang lebih penting — **tidak ada satu pun test yang merah karenanya**. `docs-consistency.test.js` (v97) cuma menjaga enam angka (`CACHE_VERSION`, versi terbaru, jumlah file `src/domain`+`src/ui`, tabel/RPC `sql/schema.sql`, cek tiap harness, versi `vendor/supabase-js-*`); semua klaim naratif berada di luar jangkauannya. Dokumen di repo ini alat kerja utama tiap sesi, jadi angka salah = keputusan salah.

**TEMUAN UTAMA (diukur ulang dari kode, bukan dari dokumen):**
- **`src/bootstrap/app.js`, `src/bootstrap/loader.js`, `src/auth/lifecycle.js` = DEAD CODE.** Tidak diimpor `boot.js` (nol sebutan `bootstrap`/`lifecycle` di berkas itu), di-tree-shake habis dari `boot.bundle.js` (`createAppBootstrap` / `createBootstrapLoader` / `createAuthLifecycle` = 0 kemunculan), dan tidak punya satu pun test unit. `createAuthLifecycle` cuma ikut ter-re-export di barrel `src/auth/index.js`. STRUKTUR-REPO §3 selama ini menggambarkannya sebagai jalur boot produksi — menyesatkan kontributor/agen ke file yang salah.
- **Jalur boot produksi yang SEBENARNYA:** IIFE `(async function bootstrapAuth() { … })()` di `app.src.js:8400` (ikut ter-minify ke dalam classic script `app.js`). Urutannya `initSupabaseClient()` → `initLoginForm()` (dulu, supaya `hideAuthMessages`/`setAuthMode` terisi sebelum `await` pertama — menghindari temporal dead zone) → `onAuthStateChange` → `getSession()` → `enterApp(session)` (gerbang App Lock v92) atau `showLoginView()`.
- **Listener itu SENGAJA hanya bereaksi ke `SIGNED_OUT`** — alasannya tertulis di `app.src.js:8418-8423`: `createAuthLifecycle()` memanggil `onAuthenticated` pada SETIAP event ber-session termasuk `TOKEN_REFRESHED` berkala, yang berarti `showAppShell()`+`initApp()` — dan seluruh `loadData()` — terulang tiap refresh token. Jalur SIGNED_OUT: `appLockOnSignedOut()` → bersihkan `_pendingTxMutations` + `_txFetchInFlight` (v119) → `showLoginView()` (`app.src.js:8320`) → `resetAppState()` + `clearOfflineDataCache()`.
- **Proteksi race ada di dalam monolit itu sendiri**, bukan di `src/bootstrap/`: generation counter `_loadDataSeq` (v69, dideklarasi `app.src.js:875`, dinaikkan juga di `showLoginView()`), plus `_txFetchInFlight`/`_pendingTxMutations` (v119) yang direkonsiliasi `reconcileTxRowsWithPending()`.
- `loadData()` mengambil **7 grup** dalam satu `Promise.all` (`app.src.js:4490`: transactions, budgets, assets, customIcons, settings, recurring, platformLogos v86); **4 di antaranya dibungkus `optional()`** → kegagalan cuma `console.warn` + fallback, bukan boot yang macet.

**PERUBAHAN:**
- **`README.md` + `STRUKTUR-REPO.md`: 16 klaim dikoreksi** — `onclick=` "200+" → **0** (data-action sejak v101-v102); `<script type="module">` memuat `boot.js` → **`boot.bundle.js`** (v103) dan berkas itu kini terdaftar di tree §2; "50+ file uji" → **89**; "ESLint 9" → **10**; konfigurasi Supabase "di `index.html`" → **`app.src.js:129`** (`SUPABASE_URL`/`SUPABASE_ANON_KEY`) dan **`:1875`** (`WHATSAPP_BOT_NUMBER`, masih placeholder); "semua 8 modal" → **19** `role="dialog"` (+overlay App Lock), semua ber-`aria-modal="true"`; upload gambar "maks 1MB" → **8MB** mentah → 480px JPEG q82 (PNG bila transparan) dan foto struk AI **10MB** → 1600px q85; tree README **2 → 5** Edge Function (+`_shared/`); AI "otomatis tiap 3 menit" → **manual-only** (kuota insights/monthly 20/jam, suggest 60/jam, question 40/jam + jeda 8 detik); CSP "hanya Supabase" → `script-src`/`connect-src` memang Supabase-only tapi **`img-src` 12 host**; `app.js` "~223KB" → **265.5KB** (`app.src.js` 575.629 B, **−52,8%**); `tailwind.css` "~50KB" → **55.0KB**; "~385 commit" → **412** (GitHub API — checkout `--depth 1`). §3 butir 4 dan tree §2 ditulis ulang mengikuti temuan dead code di atas.
- **Blok "Peta cepat" `AGENT-HANDOFF.md`** (bagian hidup, dibaca pertama tiap sesi): "SPA `index.html` + `src/**`" → memuat **dua** berkas build (`boot.bundle.js` module + `app.js` classic); Edge Functions **2 → 5** (semua nama disebut); "**7** view (…/akun/…)" → **9** view (7 di nav: dashboard/transaksi/budget/laporan/aset/kalender/pengaturan + 2 sub-view drill-down: `akun-detail`, `kategori-detail`) + **19** modal. Entri `## vNN` di bawahnya TIDAK disentuh (log historis).
- **6 dokumen historis di `docs/` diberi blockquote catatan ber-tanggal, isi aslinya TIDAK diubah** (mengubahnya = memalsukan riwayat): `current-data-flow-map.md` (sudah tak ada CDN; vendoring v59; PostgREST terkonfirmasi), `schema-contract-audit.md` (gap `mata_uang`/`kurs`/`jumlah_idr` tertutup di v95; aturan rilisnya dipenuhi job CI), `production-loader-contract.md` (`getSyncData` di-inline; 7 grup; kontrak dipenuhi monolit `app.src.js`, bukan `src/bootstrap/`), `architecture-modernization-plan.md` (status per fase ✅/⚠️ — Phase 4 **SEBAGIAN**), `supabase-native-migration-plan.md` (tujuan tercapai; release gate dipetakan ke `sql/functional-check.sql` **CEK 1a-9**; gap lintas-mata-uang level DB tetap terbuka), `SESSION-HANDOFF.md` (snapshot 2026-08-31 → tunjuk STRUKTUR-REPO + Peta cepat).
- **`tests/unit/docs-consistency.test.js` 6 → 16 test (+288 baris, total 447).** Guard baru: (1) `markup: 0 handler inline, dan SEMUA data-action terdaftar di registry` — mem-parse registry `__uiActionsCache` (169 aksi) lalu menuntut 86 `data-action` statis di markup **dan** 31 aksi dinamis hasil `uiActionAttrs()` semuanya terdaftar; (2) lokasi konstanta Supabase + nomor bot (wajib `app.src.js`, **dilarang** muncul di `index.html`) + jangkar kalimat di README §7 & STRUKTUR §2; (3) jumlah `role="dialog"` = `aria-modal="true"` = klaim "semua N modal", jumlah view unik, dan klaim gabungan "N view + M modal" di 3 tempat; (4) jumlah file `tests/unit/*.test.js` + jumlah **dan tiap nama** Edge Function; (5) `index.html` memuat `boot.bundle.js` (bukan `boot.js`) + total modul `src/` = 71 di tiga dokumen; (6) major versi ESLint dari `package.json`; (7) `onclick=` hanya boleh disebut dalam konteks larangan/historis; (8) batas upload 8MB/10MB; (9) **status wiring `src/bootstrap/` dua arah**; (10) test meta yang menghitung `test(` di dirinya sendiri.
- **`STRUKTUR-REPO.md` §5 butir 9 ditulis ulang** — dulu malah menutup dengan "Yang TIDAK dijaga test itu: klaim naratif (nama berkas tempat sebuah konstanta berada, jumlah modal, batas ukuran upload, dsb)", yaitu persis hal-hal yang kini sudah dijaga.

**GOTCHA guard baru (jangan dilemahkan tanpa alasan tertulis):**
- **Satu mutasi sempat LOLOS dan langsung dijadikan pelajaran:** mengganti `TANPA atribut onclick=` → `MASIH ADA 200+ atribut onclick=` tidak terdeteksi, karena baris itu kebetulan masih memuat kata `TANPA` di klausa sebelumnya ("TANPA konfigurasi Supabase"). Guard-nya dipertebal jadi **dua lapis**: (a) ANGKA apa pun yang menempel pada frasa "N atribut `on…=`" harus = jumlah handler inline nyata di `index.html` (kini 0), dan (b) tiap penyebutan `onclick` wajib berada dalam jendela ±60 karakter yang memuat kata larangan/historis.
- Guard (1) di atas **bukan soal dokumen melainkan soal markup**: tanpa entri registry, tombol mati saat diklik **tanpa error apa pun** — CSP `script-src` memakai 4 hash `sha256` dan menolak `unsafe-inline`, jadi handler inline juga hilang diam-diam.
- Guard wiring `src/bootstrap/` **dua arah**: kalau suatu saat berkas itu benar-benar di-wire, dokumen yang masih menulis "⚠️ BELUM ter-wire ke produksi" ikut merah (dan sebaliknya).
- Test meta menghitung `test(` di dirinya sendiri → **menambah guard baru WAJIB menaikkan angka "(N test)"** di STRUKTUR-REPO §5 butir 9.
- Parser registry memakai pencocokan kurung kurawal, bukan regex: kalau `__uiActionsCache = {` berubah bentuk, guard-nya gagal keras dengan pesan "bentuk registry berubah?" — itu disengaja, jangan di-`skip`.

**BELUM DIPUTUSKAN (butuh keputusan pemilik repo, sengaja TIDAK dikerjakan di v122):** wire-atau-hapus `src/bootstrap/` + `lifecycle.js` (kalau wire: selesaikan dulu filter `TOKEN_REFRESHED`, lalu port `_txFetchInFlight`/`_pendingTxMutations`/`reconcileTxRowsWithPending` v119 yang belum ada padanannya di sana); inkonsistensi warna PWA nyata (`<meta name="theme-color">` `#05070f` menimpa manifest `theme_color` `#151928`, padahal `background_color` `#f8fafc` — address bar Android gelap, splash terang); komentar basi `src/ui/insights.js:171-173` ("cuma manggil Gemini kalau belum ada cache" — tidak pernah auto-call; hanya bisa diperbaiki lewat `npm run build:app`); `recurring_transactions` belum punya kolom mata uang untuk Transfer lintas mata uang; `supabase/functions/analyze-finance/index.ts:19-22` masih mencatat Edge Function live `smooth-processor` yang tidak dipanggil dari mana pun.

**VERIFIKASI:** unit **948/948** (0 fail, **0 skip** — 3 test yang dulu skip kini ikut jalan setelah `npm ci`, 267 paket); `npx eslint .` **0 masalah**; `docs-consistency` **16/16**; **26 mutation test** sengaja (mis. `19 modal`→`8`, `5 Edge Function`→`2`, `ESLint 10`→`9`, `89 file test`→`50`, `71 modul`→`70`, `maks 8MB`→`1MB`, "BELUM ter-wire"→"sudah ter-wire", hapus satu nama Edge Function dari Peta cepat, tambah `onclick="alert(1)"` ke `<body>`, tambah `data-action` tak terdaftar, ganti `uiActionAttrs('setThemeColor')` dengan aksi fiktif, muat `boot.js` alih-alih `boot.bundle.js`) — **semuanya membuat suite merah**, lalu dokumen dipulihkan dan diverifikasi ulang hijau. **TIDAK ada perubahan runtime/aset** → `CACHE_VERSION` tetap `myfinance-v152`, snapshot SW tidak disentuh, tidak perlu `build:app`/`build:boot`/`build:css`.

**STATUS:** siap di-commit & push sebagai v122.

## v123 — audit bug menyeluruh: 1 bug nyata (transfer IDR-ke-IDR GAGAL di database), 2 jebakan laten diperbaiki, 1 celah guard ditutup, 16 situs formatter Intl di-cache
**KONTEKS:** permintaan pemilik repo: (1) cek potensi bug menyeluruh, (2) cek stabilitas, (3) tingkatkan performa dengan hati-hati. Audit dilakukan terhadap 8.560 baris `app.src.js`, 9.017 baris `src/**`, 1.287 baris Edge Functions, dan 4 RPC di `sql/schema.sql`.

**BUG #1 (NYATA, dampak tinggi) — transfer IDR-ke-IDR ditolak database.** Dua lapisan punya kontrak yang saling bertentangan dan keduanya HIJAU:
- JS (`src/services/supabase/transfers.js` → `toTransferParams`: `data.mata_uang_sumber || null`) SENGAJA mengirim `p_mata_uang_sumber = NULL` untuk akun IDR. Itu konvensi seluruh app: `app.src.js` `currentTxMataUang = null` (baris ~3375/~3423), kolom `transactions.mata_uang` nullable. Kontrak ini bahkan ditegaskan eksplisit oleh `tests/unit/rpc-param-shapes.test.js` — "transfer IDR-ke-IDR biasa (**mayoritas transfer di app ini**)".
- SQL (`create_transfer_transaction`) menolak NULL: `if nullif(trim(p_mata_uang_sumber), '') is null or ... then raise exception 'Mata uang sumber dan tujuan wajib diisi'`.
- **Kenapa tidak pernah ketahuan:** unit test memakai MOCK client (tidak pernah menyentuh Postgres), dan `scripts/schema-verify/functional-check.sql` CEK 3 hanya menguji transfer LINTAS mata uang (USD→IDR). Jadi gerbang Postgres di CI tidak pernah menjalankan jalur mayoritas. `docs/supabase-native-migration-plan.md` malah menandai gate "Transfer IDR→IDR … terverifikasi" sebagai terpenuhi, dengan catatan "sisa kecil: pasangan same-currency belum punya kasus uji eksplisit" — ternyata itu bukan sisa kecil, itu kegagalan total.
- **PERBAIKAN:** RPC menormalkan `NULL`/`''` → `NULL` (TETAP IDR implisit — sengaja TIDAK ditulis `'IDR'` supaya baris Transfer konsisten dengan baris Pemasukan/Pengeluaran) dan kurs `NULL` → 1; perhitungan memakai nilai ternormalisasi. Validasi kurs **tidak** ikut dilonggarkan: eksplisit 0/negatif tetap ditolak. Diterapkan IDENTIK di `sql/schema.sql` + `sql/migrations/migration_transfer_currency_2026-08.sql` (kanonik) karena `sql-schema-completeness.test.js` menuntut body byte-identical.
- **BUKTI:** CEK 3c–3i baru di `functional-check.sql` (NULL / string kosong / campuran USD→NULL / kurs 0 tetap ditolak) dijalankan di Postgres 17 nyata oleh job CI `Schema install check`; plus `tests/unit/currency-idr-contract.test.js` yang menjahit kedua lapisan (sudah diuji-balik: mengembalikan SQL ke bentuk lama membuat test ini MERAH).

**BUG #2 (LATEN, sudah terpasang tapi belum bisa terpicu) — `jumlah_idr` transaksi berulang menyalin nominal mentah.** `create_recurring_transaction` menyimpan `coalesce(p_jumlah_idr, p_jumlah)`. Lapisan JS (`src/services/supabase/recurring.js`) SUDAH meneruskan `p_mata_uang`/`p_kurs`, jadi begitu `recurring_transactions` punya kolom mata uang (gap yang tercatat di `docs/supabase-native-migration-plan.md`), template USD 100 @16.000 akan tersimpan dengan `jumlah_idr = 100` — persis regresi klasik "USD 100 jadi Rp 100" yang sudah dijaga di jalur transfer (CEK 3a/3b) tapi belum di jalur berulang. **PERBAIKAN:** `coalesce(p_jumlah_idr, case when p_kurs is not null then p_jumlah * p_kurs else p_jumlah end)`. Untuk SEMUA pemanggil hari ini (`p_kurs` selalu NULL) hasilnya IDENTIK. Bukti: CEK 4c–4e (kurs asing / jalur IDR / `p_jumlah_idr` eksplisit tetap menang).

**BUG #3 (LATEN) — `roundMoney()` di `src/domain/finance.js` asimetris dan bisa mengembalikan NaN.** (a) `Math.round()` JS membulatkan `.5` ke +Infinity: `roundMoney(2.5,0)` = 3 tapi `roundMoney(-2.5,0)` = **-2**, dan `roundMoney(-1.005,2)` = **-1** sementara sisi positifnya 1.01 → sebuah pengeluaran dan pembalikannya TIDAK saling meniadakan (sisa satu satuan terkecil, terakumulasi di laporan). (b) `decimals` tidak tervalidasi: `roundMoney(1.5, NaN)` → **NaN** (sumber "Rp NaN" di UI), `roundMoney(1.5, 1.7)` → 1.4964467362266598, `roundMoney(1.5, -1)` → 0 — semuanya tanpa keluhan. **PERBAIKAN:** bulatkan nilai absolut lalu kembalikan tandanya (half away from zero), normalkan `-0` → `0`, dan tolak `decimals` non-integer / di luar 0–15 dengan error eksplisit. Fungsi ini belum dipakai runtime (hanya diekspor + diuji, untuk Phase 4) — jadi kedua cacat diperbaiki SEBELUM ada pemanggil yang mewarisinya. 3 test baru di `tests/unit/finance-domain.test.js`.

**CELAH GUARD (ditemukan saat audit, warisan v122 saya sendiri) — 67 atribut `data-on-*` tidak diawasi.** Guard v122 `markup: 0 handler inline…` hanya memeriksa `data-action` (119 atribut). Padahal dispatcher `UI_EVENT_ATTR` (`app.src.js:1478-1485`) melayani ENAM atribut lain: `data-on-change` (29), `data-on-input` (21), `data-on-submit` (7), `data-on-keydown` (6), `data-on-focus` (2), `data-on-blur` (2). Komentar di `app.src.js:1470-1474` justru menjelaskan bahayanya: handler yang tidak terdaftar membuat fitur mati "tanpa error, cuma diam". **PERBAIKAN:** guard diperluas membaca daftar atribut DARI `UI_EVENT_ATTR` di sumbernya (menambah event ketujuh/kedelapan otomatis ikut terjaga). Kondisi sekarang: 188 atribut deklaratif, semuanya terdaftar di registry 169 aksi — tidak ada handler mati.

**PERFORMA (hati-hati, output byte-identik) — 16 situs `new Intl.NumberFormat('id-ID').format(x)` di monolit diarahkan ke `formatRp()` yang formatter-nya ter-cache.** Optimasi v112 sudah melakukan ini untuk `formatInputRibuan()` dan `src/ui/{budgets,goals-debts}.js`, tapi 16 situs lain tertinggal: hint kurs (3413), field nominal transaksi (3842/3892), modal aset (4189/4191), sub-input budget (6683/6693/6769/6774 — jalur PER-KEYSTROKE dan per-baris), layar goal (6897/6899), utang (7031/7033/7036), toast setoran/pembayaran (6962/7099). Terukur di Node 20: **744 ms vs 16 ms untuk 20.000 pemanggilan = 46× lebih cepat** (±36 µs per pembuatan formatter). Keamanan perubahan: `new Intl.NumberFormat('id-ID')` dan `new Intl.NumberFormat('id-ID',{minimumFractionDigits:0})` sama-sama memakai `maximumFractionDigits` default 3 → output diverifikasi IDENTIK untuk 23 nilai (0, negatif, desimal, NaN, Infinity, 1e15). Shim default `__fmt` (baris 163) SENGAJA tidak diubah: itu jalur sebelum `adoptFormatModule()` dan harus tetap byte-compatible. `app.js` 265,5 KB → **265,1 KB** (−464 byte).

**STABILITAS (diperiksa, hasilnya BERSIH — dicatat supaya tidak diaudit ulang dari nol):** jalur gagal `loadData()` sudah benar (saksi `_txFetchInFlight` dilepas di semua jalur keluar, panggilan basi tidak berhak menampilkan toast, `setHudStatus('error')` + `setSyncLoading(false)`); `resetAppState()` mereset `_recurringProcessed` sehingga ganti akun tanpa reload tidak melewatkan transaksi berulang; `processDueRecurring()` punya flag kegagalan PER-ITEM (bukan global), RPC idempoten di level DB, dan jadwal hanya dimajukan bila semua periode berhasil; 4 fallback `optional()` tidak membuat boot macet; `whatsapp-webhook` fail-CLOSED (`if (!WEBHOOK_SECRET || !constantTimeEqual(...))` → 401) dan memakai perbandingan constant-time; 7 `catch {}` kosong semuanya cleanup defensif (`destroy()`/`focus()`/`removeItem`), bukan error yang ditelan; tidak ada `await` di dalam `forEach`; tidak ada `new RegExp(input_user)`; pembagian selalu dijaga `total > 0 ? … : …`.

**VERIFIKASI:** unit **956/956** (0 fail, 0 skip); `npx eslint .` **0 masalah**; keempat build idempoten dan `git diff --exit-code` bersih untuk `css/tailwind.css`, `styles.css`, `index.html` (CSP tidak berubah — tidak ada blok skrip inline yang disentuh); `app.js` + `boot.bundle.js` di-rebuild dan ikut di-commit; `CACHE_VERSION` `myfinance-v152` → **`myfinance-v153`**, snapshot SW diregenerasi (`0f523f6e682d10f8…`). Test baru: `currency-idr-contract.test.js` (6) + 3 di `finance-domain.test.js` → total file test unit **89 → 90** (angka di README §1 dan STRUKTUR-REPO §2 ikut diperbarui oleh guard-nya sendiri). Bukti Postgres nyata (CEK 3c–3i, 4c–4e) berjalan di job CI `Schema install check`.

**CATATAN OPERASIONAL:** perubahan SQL ini TIDAK otomatis berlaku di database live. Terapkan dengan menjalankan ulang `sql/schema.sql` (aman: `create table if not exists` + `create or replace function`, dan CI membuktikan instalasi dua kali bersifat idempoten) ATAU jalankan `sql/migrations/migration_transfer_currency_2026-08.sql` + `sql/migrations/migration_reliability_hardening_2026-08.sql`. ~~Sampai itu dilakukan, transfer IDR-ke-IDR di produksi tetap gagal.~~ **Klausa terakhir itu SALAH dan sudah dikoreksi oleh penerapannya sendiri — lihat "SUSULAN v123" di bawah: function yang tayang di produksi ternyata bukan versi repo, jadi transfer IDR-ke-IDR di live tidak pernah gagal.**

**STATUS:** di-commit & push sebagai v123 (`405f7a1`); SQL-nya SUDAH diterapkan ke database live dan diverifikasi di sana (lihat SUSULAN).

### SUSULAN v123 (2026-09-12) — SQL diterapkan ke database LIVE, dan drift skema live-vs-repo ketemu

Pemilik menjalankan perbaikannya lewat Supabase Management API (project `uxfngmxghupdlwoeoxgh`, ap-southeast-1, PostgreSQL 17.6). Catatan: `service_role` key TIDAK BISA dipakai untuk DDL (`/database/query` menjawab 401 "JWT failed verification", dan PostgREST memang tidak menjalankan DDL) — butuh personal access token `sbp_` atau koneksi langsung. Yang terjadi di lapangan:

1. **`create_recurring_transaction` live == repo versi lama (`7054b37`).** Perbaikan #2 masuk bersih. Aman untuk lalu lintas hari ini karena app selalu mengirim `p_jumlah_idr` eksplisit — nilai simpannya identik; yang berubah hanya fallback saat `p_jumlah_idr` tidak dikirim.

2. **`create_transfer_transaction` live TIDAK cocok dengan berkas SQL mana pun di repo** — bukan foundation, bukan `migration_transfer_currency_2026-08.sql`, bukan `schema.sql` di `4343d07`/`7054b37`/`405f7a1`. Itu varian KETIGA yang hanya pernah dibuat langsung di database: pesan error bahasa Inggris (`akun_sumber and akun_tujuan are required`), tanpa cek `auth.uid()`, tanpa cek `p_tanggal`, tanpa cek "mata uang wajib", matematika `v_source_idr := p_jumlah * coalesce(p_kurs_sumber,1)` lalu `v_target_amount := v_source_idr / coalesce(p_kurs_tujuan,1)` (kurs tujuan 0 → division by zero), dan `p_kurs_sumber numeric DEFAULT 1` / `p_kurs_tujuan numeric DEFAULT 1`. **Konsekuensinya penting: transfer IDR-ke-IDR di produksi selama ini TIDAK rusak.** Bug #1 hanya mengenai instalasi baru yang dibangun dari `schema.sql` — repo dan produksi sudah menyimpang diam-diam.

3. **Postgres menolak `CREATE OR REPLACE`** dengan `cannot remove parameter defaults from existing function` (menambah default boleh, menghapus tidak). Karena `DEFAULT 1` di live selaras dengan aturan "kurs NULL = IDR implisit" dan tidak mengubah perilaku app (app selalu mengirim kurs eksplisit; semua pembaca `kurs` toleran-NULL: `src/services/parity/transactions.js:19` `row.kurs != null ? … : null`, `src/services/transactions.js:83/108` `data.kurs || 1`, `:157/177` `data.kurs || null`), **repo yang disesuaikan ke live** — bukan sebaliknya, dan bukan lewat `DROP FUNCTION` yang akan ikut menghapus grant `EXECUTE`. Kedua berkas SQL kini memuat `p_kurs_sumber numeric default 1` / `p_kurs_tujuan numeric default 1` beserta komentar alasannya. `CREATE OR REPLACE` lalu diterima.

4. **Verifikasi di live.** Body kedua function sekarang identik byte-per-byte dengan `sql/schema.sql` (sha256 `7f7c9879699fd8cc` transfer, `76ccd1efa9aaefcc` berulang), `security invoker` + `set search_path = public` utuh, grant tidak berubah (`EXECUTE` anon=false / authenticated=true), dan seluruh `DEFAULT` parameter bertahan. Uji fungsional 7 kasus dijalankan sebagai user nyata **di dalam blok yang sengaja di-`raise` supaya rollback otomatis**: IDR-ke-IDR (NULL) 12345 → 12345 dengan `mata_uang` tersimpan NULL dan `kurs` 1, string kosong → NULL, USD 100 @16000 → 1600000, kurs 0 tetap ditolak, sumber=tujuan tetap ditolak, berulang USD → 1600000 dan IDR → 99000, idempotensi berulang tetap 1 baris. Hasil `SEMUA_UJI_LULUS (7/7)`; pengecekan sesudahnya `sisa_uji: 0` dari 146 baris `transactions` — nol data tertinggal. Rekaman body live sebelum ditimpa disimpan di `/home/user/ROLLBACK-v123-database-live.sql` (di luar repo) kalau perlu dikembalikan.

**PELAJARAN YANG WAJIB DIBAWA (dan belum ada penjaganya):** `sql-schema-completeness.test.js` hanya membandingkan berkas-berkas DI REPO satu sama lain, jadi **drift live-vs-repo tidak terlihat oleh CI mana pun** — dua-duanya hijau sementara yang tayang beda. Kalau ada objek lain yang pernah diutak-atik langsung di database (trigger, kebijakan RLS, index, kolom), kondisinya belum terperiksa. **Usulan (BELUM dikerjakan, tunggu keputusan pemilik):** job CI `workflow_dispatch` yang membandingkan `pg_proc.prosrc` + `information_schema` dari database live terhadap `sql/schema.sql`, memakai secret, sehingga tidak perlu menempel di setiap push. **Kredensial yang dipakai sesi ini (service_role key + token `sbp_`) pernah ditempel di chat — keduanya harus dirotasi.**

## v124 — audit drift live-vs-repo: produksi vs `schema.sql` disamakan, dan celah penjaganya ditutup
**KONTEKS:** v123 menemukan secara tidak sengaja bahwa body `create_transfer_transaction` yang tayang tidak cocok dengan berkas SQL mana pun di repo. Pemilik repo lalu meminta **audit menyeluruh secara bertahap dan teliti** atas seluruh objek database. Hasil lengkap: `docs/audit-drift-live-vs-repo-2026-09-13.md` (12 temuan). Seluruh pengumpulan data read-only; perubahan ke produksi baru dilakukan setelah pemilik memilih paket remediasinya.

**METODE (yang membuat hasilnya bisa dipercaya, dan layak diulang):** membandingkan teks SQL dengan regex itu rapuh, jadi database REFERENSI dibangun sungguhan — biner PostgreSQL 17.5 tertanam (tanpa root), database kosong, `supabase-shim.sql`, lalu `sql/schema.sql` **dua kali**. Katalog kedua sisi di-dump dengan **query yang identik**. Perbandingan dilakukan **per definisi, bukan per nama**; body function dibandingkan **setelah komentar dibuang dan CRLF/whitespace dinormalkan**.

**TEMUAN PENTING:**
- **F1 — `platform_logos.id` `bigint GENERATED BY DEFAULT AS IDENTITY` di produksi, `uuid` di repo.** Repo konsisten internal (schema.sql dan migration kanonik sama-sama `uuid`); yang menyimpang produksinya. Akarnya: tabel dibuat lewat **Supabase Dashboard UI** SEBELUM migration-nya dijalankan, lalu `create table if not exists` dan `alter table add column if not exists` **diam-diam jadi no-op**. Migration-nya sendiri sudah mengakui ("tahan drift bentuk vs tabel live yang dibuat migrasi yang hilang tadi"). **Dampak runtime NIHIL** — `src/services/supabase/platform-logos.js` hanya membaca `platform_key, display_name, logo_url, source_url`, tidak pernah `id`. **Pelajaran umum: `if not exists` membuat migration TAHAN DIJALANKAN ULANG tapi TIDAK KOREKTIF.**
- **F2 — CHECK `transfer_target_idr_nonnegative` hilang di produksi.** Kolomnya ada, constraint-nya tidak. Satu-satunya hal yang membuat produksi lebih lemah dari repo.
- **F3 — empat CHECK di `transactions` ada di produksi tapi bernama lain.** Perbandingan berbasis nama menyimpulkan "5 hilang"; berbasis definisi yang benar: **1 hilang**. Audit skema yang membandingkan `conname` akan menghasilkan temuan palsu.
- **F4/F5/F6 — trigger + function `set_platform_logos_updated_at`, index `platform_logos_active_idx`, dan CHECK `platform_logos_key_format` hanya ada di produksi.** Frasa `set_platform_logos_updated_at` tidak muncul di folder `sql/` sama sekali. Index itu melayani PERSIS query aplikasi (`.eq('is_active', true).order('display_name')`).
- **F8 — `check_and_consume_rate_limit` md5-nya beda dan body live lebih pendek 1679 byte. ALARM PALSU.** Setelah komentar dibuang: sha256 **identik**, 987 karakter keduanya. Kedua guard keamanannya (`auth.role() = 'anon'`, `auth.uid() <> p_user_id`) ada di produksi. Hampir saja dilaporkan sebagai celah.
- **F11 — guard anti-drift repo LOLOS PALSU.** `sql-schema-completeness.test.js` memeriksa kolom dengan `new RegExp("\b" + column + "\b").test(schema)` — mencari nama kolom di SELURUH berkas, **tidak dibatasi ke tabel pemiliknya**. Terbukti lewat mutation test: menghapus `updated_at` dari `create table platform_logos` membuat **seluruh 956 test tetap hijau**, padahal kolom itu `not null` dan seed-nya akan gagal. Inilah alasan F1–F6 bertahan lama: **tidak ada penjaga yang membandingkan definisi, hanya nama.**
- **F12 — `schema.sql` memasang DUA PASANG CHECK duplikat di `transactions`**, dan komentarnya membenarkan itu dengan klaim **"KEDUANYA ada di database live" — klaim itu SALAH.** Produksi hanya punya satu dari tiap aturan (keluarga nama `transactions_*`); empat nama lain (`transaction_currency_rate_positive`, `transaction_amount_idr_nonnegative`, `transfer_target_amount_positive`, `transfer_target_rate_positive`) tidak pernah ada di produksi. Instalasi baru mendapat 11 constraint di `transactions` vs produksi 9, dengan dua aturan identik dievaluasi dua kali per INSERT/UPDATE. **Komentar yang salah lebih berbahaya daripada tidak ada komentar: ia menutup pertanyaan.**
- **BUKAN drift (jangan diaudit ulang dari nol):** `table_grants` 13 vs 44 adalah **keterbatasan shim yang terdokumentasi** (tidak meniru default-privilege Supabase), dan **RLS aktif di 11/11 tabel** menutupnya — `api_rate_limits` punya 0 policy jadi tetap tak terbaca. `rls_auto_enable` + event trigger `ensure_rls` hanya di produksi **secara sengaja** (schema.sql baris 54-57: OPSIONAL, butuh superuser). Default `now()` vs `timezone('utc', now())` dan UNIQUE constraint vs unique index **setara semantik**. Edge Function **5 = 5** semua ACTIVE dengan `verify_jwt=true`; storage bucket 0.

**YANG DIJALANKAN (pemilik memilih "semua paket"):**
- **Produksi, 2 statement tertarget** — `sql/schema.sql` SENGAJA tidak dijalankan ulang ke produksi: seed-nya `on conflict do update` akan menyentuh `updated_at` ke-11 logo, dan `drop policy` + `create policy` menyisakan jendela singkat tanpa policy. Ditambah CHECK `transfer_target_idr_nonnegative` (prasyarat diuji dulu: **0 dari 147 baris melanggar**) dan dihapus policy redundan `platform_logos_read_authenticated` (permissive digabung OR dengan `using (true)`, jadi tidak membatasi apa pun; policy itu warisan migrasi yang hilang dan disebut di header migration-nya sendiri). Hasil: **total policy produksi 16 → 15 = repo**, data utuh.
- **Repo — `schema.sql` kini menggambarkan produksi**: `platform_logos` jadi `bigint identity` + CHECK format kunci + UNIQUE constraint + default `timezone('utc', now())`, ditambah index `platform_logos_active_idx` dan trigger + function `set_platform_logos_updated_at`. Arahnya **repo mengikuti live**: memigrasikan `id` produksi ke `uuid` berarti menulis ulang tabel + membuang sequence, berisiko, dan manfaatnya nol karena tidak ada pemakai kolom itu. Constraint `transactions` disamakan 7 → 5 (F12), dengan keempat nama yang tidak pernah ada di live tetap di-`drop` supaya database lama ikut bersih. `migration_transfer_currency_2026-08.sql` dan `20260906_platform_logos.sql` diberi anotasi; yang kedua tetap ARSIP dan tidak ditulis ulang.
- **Angka objek ikut diperbarui di TIGA tempat** (mudah terlewat): `EXPECTED` di `scripts/schema-verify/run.mjs` `11/4/15` → **`11/5/15`**; `docs-consistency.test.js` kini memisahkan **RPC (4)** dari **fungsi trigger (1)** supaya klaim "4 RPC" di README tetap benar; README + STRUKTUR-REPO disesuaikan.
- **Guard diperketat + alat baru.** `sql-parse.mjs` jadi parser bersama (pencocokan **kurung seimbang**, bukan regex — regex pertama GAGAL karena komentar v123 di dalam arg-list memuat tanda `)`). Guard kolom kini **dibatasi ke tabel pemiliknya** + ada **kontrak tipe kolom** terhadap produksi + guard objek `platform_logos`. Baru: `drift-check.mjs` (`--check` di CI, `--live-check` manual dengan `SUPABASE_ACCESS_TOKEN`), `catalog-queries.json` (satu sumber untuk kedua sisi), `expected-catalog.json` (snapshot), dan `tests/unit/drift-check-contract.test.js` (5 guard — yang terpenting: **semua query katalog wajib read-only**, karena isinya dieksekusi ke produksi dengan token level akun).

**VERIFIKASI:**
- **Uji asam:** referensi dibangun ulang dari `schema.sql` v124 → `tables/columns/policies/indexes/constraints/triggers/sequences` semuanya **IDENTIK** dengan produksi; `functions` 5 vs 6 dengan **satu-satunya selisih adalah `rls_auto_enable` yang memang diizinkan**. `drift-check.mjs --live-check` terhadap produksi sungguhan **keluar kode 0**.
- **14 uji negatif, semuanya membuat merah** (lalu berkas dipulihkan): hapus `updated_at` (yang dulu lolos palsu → kini 3 test merah), `id`→uuid, hapus trigger/index/CHECK, default→`now()`, selipkan `delete`/`update` ke `catalog-queries.json`, tempel token `sbp_` ke alat, buat snapshot basi, dan 4 penyimpangan lewat alat drift — **termasuk menghapus satu policy RLS `transactions` dan melonggarkannya jadi `using (true)`. Kebocoran RLS kini membuat CI merah; sebelum v124 tidak ada penjaga yang bisa melihatnya.**
- Unit **963 test** (91 file, dari 90); `docs-consistency` **16/16**; `sql-schema-completeness` **5 → 7**; `drift-check-contract` **5** baru. `npx eslint .` **0 masalah**. Tidak ada perubahan runtime/aset → `CACHE_VERSION` tetap **`myfinance-v153`**, tidak perlu `build:app`/`build:boot`/`build:css`.
- **Yang sengaja tidak diubah:** `rls_auto_enable` tetap hanya di produksi (terdokumentasi + dijaga agar tetap disebut di schema.sql); baris `ipot` nonaktif tetap ada (app memfilter `is_active`); default privilege Supabase tidak disentuh (RLS yang menahan — mencabutnya adalah keputusan pertahanan-berlapis yang terpisah); `check_and_consume_rate_limit` tidak ditimpa ulang hanya untuk menyamakan komentar.

**BATASAN YANG JUJUR DICATAT:** paritas **kode** Edge Function tidak bisa diverifikasi — Management API tidak punya endpoint unduh bundle, jadi yang terbukti hanya slug ada + `status=ACTIVE` + `verify_jwt=true`. Di luar cakupan: konfigurasi Auth/GoTrue, `ALTER DEFAULT PRIVILEGES`, isi `vault`, dan schema non-`public`. Petunjuk yang layak ditindaklanjuti terpisah: `smooth-processor` **tidak ada** di daftar Edge Function live padahal komentar `supabase/functions/analyze-finance/index.ts:19-22` masih mencatatnya.
